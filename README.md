...
Python で撮影日時付きで CR2 から JPEG を抽出する。

## Python の外部パッケージ (Pillow/rawkit/Piexif) の問題点
* 変換が遅ぇ
...
このうち、生のデータを持っているのは Loseless JPEG で、CR2 のファイルサイズがやたら大きいのはこいつのせいです (どうも可逆圧縮の JPEG らしく... JPEG の概念が揺らぎますね)。

CR2 はメタデータの記述に、JPEG の Exif と同じ TIFF フォーマットを用いています。Image File Directory (IFD) がそれです。CR2 の詳しいフォーマットは[参考](#参考)文献に丸投げします。

一応、sample.CR2 を例に、今回の変換に関係する箇所を表にまとめておきます。
...
今回の現像では IFD#0 にある圧縮された原寸大画像を抽出するので、IFD#0 中の StripOffset と StripByteCounts を使います。`0x5a` に存在するオフセットへ飛ぶと JPEG のマーカー`0xffd8` が見えるはずです (オフセットはバイトオーダーからの距離です)。

また、撮影日時は `0x39c` に存在し、最後がヌル文字になっています。

これで画像と撮影日時の抽出が終わったので、次は現像に移ります。
...
詳しいフォーマットは[参考](#参考)文献を参照していただくとして、追加するマーカーについて表で説明します。
...
| 12 (8)  | `01 00`       | IFD#0 中のエントリ数 |
| 14 (a)  | `69 87 04 00 01 00 00 00 1a 00 00 00` | ExifOffset (0x0000001a) |
| 20 (16) | `00 00 00 00` | Next IFD offset |
| 24 (1a) | `01 00`       | SubIFD 中のエントリ数 |
| 26 (1c) | `03 90 02 00 14 00 00 00 28 00 00 00` | DateTimeOriginal (0x0000028) |
| 32 (28) | `32 30 31 39 3a 30 39 3a 32 30 20 30 36 3a 31 37 3a 34 37 00` | 2019:09:20 06:17:47\x00 |
