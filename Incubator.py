...
class Incubator:
    ...
    def make_exif(self):
        exif = \
            [ 0xFF, 0xE1 ] + list(pack('>H', 48+len(self.datetime))) + [ 0x45, 0x78, 0x69, 0x66, 0x00, 0x00, 0x49, 0x49, 0x2A, 0x00, 0x08, 0x00 ] + \
            [ 0x00, 0x00, 0x01, 0x00, 0x69, 0x87, 0x04, 0x00, 0x01, 0x00, 0x00, 0x00, 0x1A, 0x00, 0x00, 0x00 ] + \
            [ 0x00, 0x00, 0x00, 0x00, 0x01, 0x00, 0x03, 0x90, 0x02, 0x00, 0x14, 0x00, 0x00, 0x00, 0x28, 0x00 ] + \
            [ 0x00, 0x00 ] + list(self.datetime)
        return bytearray(exif)

    # Extract loosy JPEG in IFD0
    def make_loosy_jpg(self):
        offset_raw = self.find_tag_call(self.offset_1st_ifd0, 0x111,  self.get_value)
